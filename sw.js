/* Elite Scholar Institute — cache-first offline worker 36.2292 */
'use strict';
...
async function report(type, extra = {}) {
  ...
  list.forEach(client => client.postMessage({ type, ...extra }));
}
  ...
  for (const path of APP_SHELL) {
    ...
    try {
      ...
      if (existing) {
        ...
        ok = await save(cache, url, response);
      }
    } catch (_) {}
...
self.addEventListener('install', event => {
  event.waitUntil((async () => {
    // Open the new cache first. Shell warming is deliberately non-fatal:
    // one unavailable asset must NEVER leave the worker half-installed.
...
self.addEventListener('activate', event => {
  event.waitUntil((async () => {
    ...
    // Warm the app shell after activation, not before activation.
    // This prevents a failed shell download from blocking the new worker.
    await warmShell();
  })());
});
    ...
    event.waitUntil((async () => {
      ...
      for (const raw of event.data.urls) {
        try {
          ...
          await save(cache, url.href, response);
        } catch (_) {}
      }
    })());
  }
...
self.addEventListener('fetch', event => {
  ...
  event.respondWith((async () => {
    ...
    // CACHE FIRST. Keep the browser's exact URL — especially .html URLs.
    ...
    try {
      ...
      // Cache successful same-origin GET resources as the user encounters them:
      // HTML, CSS, JS, images, PDFs, fonts, JSON, video and other GET assets.
      if (response.ok) await save(cache, request, response);
      return response;
    } catch (_) {
      // Offline navigation fallback. Never rewrite a requested HTML page to '/'.
      if (request.mode === 'navigate' || request.destination === 'document') {
        const offline = await cache.match(absolute(OFFLINE_URL), { ignoreSearch: true });
        if (offline) return offline;
        return new Response(
          '<!doctype html><meta name="viewport" content="width=device-width,initial-scale=1"><title>Offline</title><body><h1>You are offline</h1><p>This page has not been cached yet.</p></body>',
          { status: 200, headers: { 'Content-Type': 'text/html; charset=utf-8' } }
        );
      }

      const retryCache = await cache.match(request, { ignoreSearch: true });
      return retryCache || new Response('', { status: 504, statusText: 'Offline' });
    }
  })());
});
