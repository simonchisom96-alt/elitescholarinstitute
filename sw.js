/* Elite Scholar Institute — cache-first offline worker 36.2293 */
'use strict';
...
async function warmShell() {
  ...
  for (const path of APP_SHELL) {
    ...
    completed++;
    await report('DOWNLOAD_PROGRESS', { completed, total: APP_SHELL.length, percent: Math.round(completed * 100 / APP_SHELL.length), file:path, ok });
  }
  ...
  event.respondWith((async () => {
    ...
    try {
      ...
      if (request.mode === 'navigate' || request.destination === 'document') {
        ...
        return new Response('<!doctype html><meta name="viewport" content="width=device-width,initial-scale=1"><title>Offline</title><body><h1>You are offline</h1><p>This page has not been cached yet.</p></body>', { status:200, headers:{'Content-Type':'text/html; charset=utf-8'} });
      }
