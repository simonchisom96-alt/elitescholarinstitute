/* Elite Scholar Institute — application controller 36.2349 */
(() => {
  'use strict';
  ...
  function releaseInitialLoader() { const loader = document.getElementById('loader'); if (loader) loader.classList.add('fade-out'); }
  function scheduleInitialLoaderFallback() { setTimeout(releaseInitialLoader, 6000); }
  ...
  function loadFcmClient(){if(document.querySelector('script[data-esi-fcm-client]'))return;const s=document.createElement('script');s.src='/fcm-client.js?v='+BUILD;s.async=true;s.dataset.esiFcmClient='1';document.head.appendChild(s);}
  async function boot(){ensureManifest();repairMobileCompatibilityCSS();createInstallUI();scheduleInstall(15000);setupBell();scheduleInitialLoaderFallback();await registerServiceWorker();loadFcmClient();}
