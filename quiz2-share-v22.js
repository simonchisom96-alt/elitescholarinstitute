/* ESI Quiz Studio Share Engine V22 — public-link foundation, backend-ready */
(()=>{'use strict';
...
function create(meta={}){const q=current(),slug=String(meta.slug||makeSlug(q.title));const rec={id:`esiq_${Date.now().toString(36)}_${rand(5)}`,slug,title:String(meta.title||q.title||'Untitled Quiz'),createdAt:Date.now(),updatedAt:Date.now(),visibility:meta.visibility||'public',mode:meta.mode||'self-paced',status:'published',responses:0};const list=[...record().filter(x=>x.slug!==slug),rec].slice(-100);write(KEY,list);return build(rec)}
function build(rec){const base=origin();return{...clone(rec),canonical:`${base}/q/${encodeURIComponent(rec.slug)}`,fallback:`${base}/quiz2.html?quiz=${encodeURIComponent(rec.slug)}`,responseUrl:`${base}/q/${encodeURIComponent(rec.slug)}`,qrValue:`${base}/q/${encodeURIComponent(rec.slug)}`,shareText:`${rec.title}\n${base}/q/${encodeURIComponent(rec.slug)}`}}
...
async function copy(slug){const x=get(slug)||create();try{await navigator.clipboard.writeText(x.responseUrl);return{ok:true,url:x.responseUrl}}catch{return{ok:false,url:x.responseUrl}}}
...
window.ESIQuizShareV22={create,build,get,list,resolve,copy,qr,unpublish,makeSlug};
